...
class SpatialMaxPoolingNHWC[T: ClassTag](
  kW: Int, kH: Int, dW: Int, dH: Int, padW: Int = 0, padH: Int = 0)
  (implicit ev: TensorNumeric[T]) extends SpatialMaxPooling[T](kW, kH, dW, dH, padW, padH) {

  def this(kW: Int, kH: Int)(implicit ev: TensorNumeric[T]) {
    this(kW, kH, kW, kH)
  }

  // return (dimc, dimh, dimw)
  protected def getDims(input: Tensor[T]): (Int, Int, Int) = {
    (input.dim(), input.dim() - 2, input.dim() - 1)
  }
  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    val (dimc, dimh, dimw) = getDims(input)
  ...
  override def equals(obj: Any): Boolean = {
    ...
    if (!obj.isInstanceOf[SpatialMaxPoolingNHWC[T]]) {
      return false
    }
    val other = obj.asInstanceOf[SpatialMaxPoolingNHWC[T]]
    if (this.eq(other)) {
      return true
    }
...
object SpatialMaxPoolingNHWC {
  def apply[@specialized(Float, Double) T: ClassTag](
      ...
      dH: Int = 1,
      padW: Int = 0,
      padH: Int = 0)(implicit ev: TensorNumeric[T]): SpatialMaxPoolingNHWC[T] = {
    new SpatialMaxPoolingNHWC[T](kW, kH, dW, dH, padW, padH)
  }
}
