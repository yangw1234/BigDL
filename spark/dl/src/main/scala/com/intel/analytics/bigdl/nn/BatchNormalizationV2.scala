...
class BatchNormalizationV2[@specialized(Float, Double) T: ClassTag](
  ...
  val runningVar = Tensor[T](nOutput)
  val saveMean = Tensor[T](nOutput)
  val saveVar = Tensor[T](nOutput)
  val invStdBuff = Tensor[T](nOutput)
  ...
  val scaleBuff: Tensor[T] = if (affine) Tensor[T](nOutput) else null
  val biasBuff: Tensor[T] = if (affine) Tensor[T](nOutput) else null
    ...
    if (train) {
      format match {
        case "NCHW" =>
          ...
          var i = 0
          while (i < N * C) {
            val sum = input2d.select(1, i + 1).sum()
            saveMeanData(i % C + saveMeanOffset) =
              ev.plus(saveMeanData(i % C + saveMeanOffset), sum)
            i = i + 1
          }
          saveMean.div(ev.fromType(N*sampleSize))

          val saveStdOffset = saveVar.storageOffset() - 1
          val saveStdData = saveVar.storage().array()
          i = 0
          while (i < N * C) {
        ...
        case "NHWC" =>
          ...
          while(i < N*sampleSize) {
            val channels = input2d.select(1, i + 1)
            varBuffer.sub(channels, saveMean)
            saveVar.addcmul(varBuffer, varBuffer)
            i = i + 1
          }
          saveVar.div(ev.fromType(N*sampleSize))
      }

      runningMean.mul(ev.fromType(1 - momentum)).add(ev.fromType(momentum), saveMean)
      runningVar.mul(ev.fromType(1 - momentum)).add(ev.fromType(momentum), saveVar)
    }

    val (mean, invStd) = if (train) {
      invStdBuff.add(saveVar).add(ev.fromType(eps)).pow(ev.fromType(-1)).sqrt()
      (saveMean, invStdBuff)
    } else {
      invStdBuff.add(runningVar).add(ev.fromType(eps)).pow(ev.fromType(-1)).sqrt()
      (runningMean, invStdBuff)
    }

    val (w, b) = if (affine) {
      scaleBuff.cmul(invStdBuff, weight)
      biasBuff.cmul(invStd, mean).cmul(weight)
      biasBuff.sub(bias, biasBuff)
      (scaleBuff, biasBuff)
    } else {
      biasBuff.cmul(mean, invStd).mul(ev.fromType(-1))
      (weight, biasBuff)
    }
    ...
    format match {
      case "NCHW" =>
        ...
        while (i < N*C) {
          ...
          outSlice.mul(inSlice, w.valueAt(i%C + 1)).add(b.valueAt(i%C + 1))
          i = i + 1
        }
  ...
  @inline
  private def calcSumOfSquare(input: Tensor[T], mean: T): T = {
    ...
    while (f < nInput) {
      ...
      results(f) = Engine.model.invoke(() => {
        ...
        if (train) {
          ...
          saveVar.setValue(_f, ev.fromType(invstd))
        ...
        if (needFix) {
          ...
          saveVar.zero().fill(ev.fromType(invstd))
        }
  ...
  def backward(input: Tensor[T], gradOutput: Tensor[T], scale: T = ev.fromType[Int](1),
    ...
    require(null != saveMean && null != saveVar, "must call updateOutput() first")
  ...
  private def backwardDouble(input: Array[Double], inputOffset: Int, inputStride: Int,
    ...
    while (f < nInput) {
      ...
      results(f) = Engine.model.invoke(() => {
        ...
        val (mean, invstd) = if (train) {
          (ev.toType[Double](saveMean.valueAt(_f)), ev.toType[Double](saveVar.valueAt(_f)))
        } else {
          (ev.toType[Double](runningMean.valueAt(_f)),
  ...
  private def backwardFloat(input: Array[Float], inputOffset: Int, inputStride: Int,
    ...
    while (f < nInput) {
      ...
      results(f) = Engine.model.invoke(() => {
        val w = if (null != weight) ev.toType[Float](weight.valueAt(_f)) else 1.0f
        val (mean, invstd) = if (train) {
          (ev.toType[Float](saveMean.valueAt(_f)), ev.toType[Float](saveVar.valueAt(_f)))
        } else {
